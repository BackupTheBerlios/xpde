...
var
    ...
    b:TBitmap;
begin
    image5.Picture.LoadFromFile(XPAPI.getSysInfo(siSystemDir)+sDESKTOPPROPERTIESMONITOR);
    b:=TBitmap.create;
    try
        ...
        b.LoadFromFile(XPAPI.getSysInfo(siSystemDir)+sBMP);
        b.width:=imgList.Width;
        b.height:=imgList.height;
        imgList.AddMasked(b,clFuchsia);

        b.LoadFromFile(XPAPI.getSysInfo(siSystemDir)+sJPG);
        b.width:=imgList.Width;
        b.height:=imgList.height;
        imgList.Add(b,b);
    finally
        ...
        try
            if reg.OpenKey('Software/XPde/Desktop/Wallpaper',false) then begin
                ...
                if path='none' then begin
                    ...
                    desktop.desktopimage:=XPAPI.getSysInfo(siUserDir)+'/.xpde/Wallpapers/Bliss1024.jpg';
                    desktop.method:=2;
            end;
        finally
            reg.Free;
...
begin
    ...
    try
        dir:=XPAPI.getSysInfo(siUserDir)+'/.xpde/Wallpapers/';
...
begin
    ...
    canvas.brush.color:=dclBtnHighlight;
    canvas.pen.color:=dclBtnHighlight;
