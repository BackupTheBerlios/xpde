...
uses
  Types, SysUtils, Classes, Qt, QTypes, QControls, QStdCtrls, QGraphics, uXPStyleConsts,
...
{ TToolBar }
  ...
  private
    ...
    procedure SetBitmap(const Value: TBitmap); reintroduce;
  protected
    FToolBar: TToolBar;
...
const
  ...
  TabDefaultTextOffsetMap: array [TTabStyle] of Integer = (-2, 1, 1, 0);
...
begin
  ...
  begin
    ...
    tw:=Tabs.TabControl.Canvas.TextExtent(Caption).cx;
    Result := tw+(TabDefaultBorderWidthMap[Tabs.TabControl.Style] * 4)+4;
  ...
  begin
    ...
    try
      ...
      while (i<rect.right) do begin
        canvas.pixels[i,rect.top]:=clBlack;
        inc(i,2);
      end;

      i:=rect.top+1;
      while (i<rect.bottom) do begin
        canvas.pixels[rect.left,i]:=clBlack;
        inc(i,2);
      end;

      i:=rect.left;
      while (i<rect.right) do begin
        canvas.pixels[i,rect.bottom-1]:=clBlack;
        inc(i,2);
      end;

      i:=rect.top;
      while (i<rect.bottom) do begin
        canvas.pixels[rect.right-1,i]:=clBlack;
        inc(i,2);
      end;
...
var
  ...
  begin
    ...
    begin
      ...
      if Erase then Pen.Color := clBackground else Pen.Color:= dclBtnShadow;
      Polyline([Point(R.Right - 2, R.Top + 2),
                Point(R.Right - 2, R.Bottom)]);

      if Erase then Pen.Color := clBackground else Pen.Color:= dclGray;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if Item <> nil then
            begin
            // why is a blank caption explicitly disallowed? bug 131376
//              if S <> '' then
                Item.Caption := S;
//              else
//                Item.Caption := FCaption;
...
begin
  inherited Create(AOwner);
  Init(TListItem);
  QFrame_setFrameStyle(handle,integer(QFrameShape_StyledPanel) or Integer(QFrameShadow_Sunken));
  QFrame_setMargin(handle,1);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      R := Types.Rect(0, 0, Width-3, Height);
...
//          False, clNone);
        drawbutton(canvas,r,(Down and not FMenuDropped), false, false);
      ...
      begin
        ...
        and (FImageIndex < FToolBar.HotImages.Count) then
          FToolbar.HotImages.Draw(Canvas, R.Left+1, R.Top-1, FImageIndex, itImage, not Ind)
        else if Assigned(FToolBar.DisabledImages) and not IsEnabled and
          (FImageIndex < FToolBar.DisabledImages.Count) then
          FToolbar.DisabledImages.Draw(Canvas, R.Left+1, R.Top-1, FImageIndex,
            itImage, not Ind)
        else if Assigned(FToolBar.Images) and
          (FImageIndex < FToolBar.Images.Count) then
          FToolBar.Images.Draw(Canvas, R.Left+1, R.Top-1, FImageIndex, itImage,
            IsEnabled and not Ind);
      end;
      if assigned(FBitmap) then begin
        FBitmap.transparent:=true;
        Canvas.Draw(r.left+1,r.top-1, FBitmap);
      end;
      ...
      begin
        ...
        if not IsEnabled then
          ...
          if Down and not FMenuDropped then canvas.font.style:=[fsBold]
          ...
          max:=length(FCaption);
          l:=1;
          while (canvas.textwidth(cp)>w) do begin
                cp:=copy(FCaption,1,max-l)+'...';
                inc(l);
                if (l=max) then break;
          end;
        Canvas.TextRect(CR, CR.Left+1, CR.Top-1, cp, DrawFlags);
      end;
    end;
...
procedure TToolButton.SetBitmap(const Value: TBitmap);
begin
...
begin
  InternalSetButtonWidth(Value, false);
end;
